"""Bullish mitigation block formation detection."""
...
class BullishMitigationDetector:
    """Identify bullish mitigation blocks from bearish origin candles."""
    ...
    def find_formations(
        ...
        """Return bullish mitigation candidates from displacement legs."""
        ...
        for index in range(max(1, lookback_start), scan_end):
            ...
            evidence = [
                f"Bullish mitigation from bearish origin at bar {index}",
    ...
    def _find_displacement(
        ...
        if displacement_index is not None:
            return displacement_index, max_high - zone_high
