"""Input validation for breaker block analysis."""
...
from backend.engines.market_breaker.schemas import BreakerBlockState
...
class ValidationResultModel:
    """Validation outcome for breaker block inputs."""
...
class BreakerBlockInputValidator:
    """Validate candles and upstream context before breaker block analysis."""
    ...
    def validate_order_blocks(
        self,
        invalidated_order_blocks: list[OrderBlock] | None,
    ) -> ValidationResultModel:
        """Validate invalidated order blocks from upstream."""
        if not invalidated_order_blocks:
            return ValidationResultModel(is_valid=True)

        errors: list[str] = []
        for index, block in enumerate(invalidated_order_blocks):
            if block.status is not OrderBlockStatus.INVALIDATED:
                errors.append(
                    f"Order block at index {index} is not invalidated: {block.block_id}",
                )

        return ValidationResultModel(is_valid=not errors, errors=errors)

    def validate_state(self, state: BreakerBlockState | None) -> ValidationResultModel:
        """Validate prior state for duplicate breaker identifiers."""
        if state is None:
            return ValidationResultModel(is_valid=True)
        ...
        seen: set[str] = set()
        for breaker in state.active_breakers:
            if breaker.breaker_id in seen:
                errors.append(f"Duplicate breaker id in state: {breaker.breaker_id}")
    ...
    def validate_or_raise(
        ...
        blocks_result = self.validate_order_blocks(invalidated_order_blocks)
        if not blocks_result.is_valid:
            raise InvalidOrderBlocksError(
                "Invalidated order block validation failed",
