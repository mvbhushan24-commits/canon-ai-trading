"""Bullish premium / discount analysis — discount territory and bullish Fib."""
...
class BullishPremiumDiscountAnalyzer:
    """Discount-side Fibonacci projection and OTE derivation."""
    ...
    def derive_ote(
        ...
        """Derive discount OTE band for bullish / long context."""
        ...
        evidence = [
            "Bullish OTE derived in discount territory",
            f"OTE band {low} – {high}",
        ]
        if overlapping:
        ...
        return OptimalTradeEntryZone(
            ote_id=f"ote-bull-{uuid4().hex[:12]}",
    ...
    def supports_trend(self, trend: TrendDirection | None) -> bool:
        """Return whether bullish discount context aligns with structure trend."""
        return trend in {TrendDirection.BULLISH, TrendDirection.RANGE, None}
    ...
    def _fib_label(ratio: Decimal) -> str:
        ...
        if ratio == Decimal("0.5"):
            return "equilibrium"
        if ratio == Decimal(str(0.618)) or ratio == Decimal("0.618"):
            return "ote_start"
        if ratio == Decimal(str(0.79)) or ratio == Decimal("0.79"):
            return "ote_upper"
        return f"fib_{ratio}"
